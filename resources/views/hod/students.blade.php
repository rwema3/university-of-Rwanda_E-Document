        ...
        var defaultUrl = "{{ route('admin.students.generalList') }}";
                ...
                // ],
                    ...
                    columns: [
                        ...
                        { var defend="";
                             if(row.has_defended){
                                defend += "<button class='btn btn-success btn-sm change-defend' data-id='"+data+"'> Defended </button>";

                            }
                            else{
                                defend += "<button class='btn btn-warning btn-sm change-defend' data-id='"+data+"' > Not  </button>";
    ...
    $('.btn-Add').on('click', function(){
        ...
        function hideLoading(){
            ...
            // Edit and update
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            console.log(response);
                              ...
                              //  set student level
                            var level = response.levels;

                            var levelOptions = "";

                            $.each(level,function (i, value){
                                if(response.staff.level_id == null && i == 0){
                                    levelOptions += "<option value=''> select Student Level </option>";

                                }
                                if(response.staff.level_id != null){
                                var passedLevel = value.id < response.staff.level_id ? 'disabled' : '';

                                    if(response.staff.level_id == value.id){
                                        levelOptions += "<option "+ passedLevel +" selected value='"+ value.id +"'>"+ value.name + "</option>";
                                    }else{
                                        levelOptions += "<option "+ passedLevel +" value='"+ value.id +"'>"+ value.name + "</option>";
                                    }
                                }
                                else
                                {
                            ...
                            if(response.staff.gender == 'M'){
                                genderOption += "<option value='M' selected > Male </option> <option value='F' > Female </option>";
                            }
                            else{
                                genderOption += "<option value='M'  > Male </option> <option value='F' selected> Female </option>";

                            }
                            $('#edit_gender').html(genderOption);
                ...
                // change defended status

                manageTable.on('click', '.change-defend', function(e){

                   var sId = $(this).attr('data-id'); 
                    

                    var url = "/hod/students/defendStatus/"+sId;

                    console.log(url);

                     
                    Swal.fire({
                    title: 'Do you want to Consider this Defended?',

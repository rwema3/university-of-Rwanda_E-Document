...
@section('page_title','departements')
...
<div class="content ">
    ...
    <div  class="row breadcrumb-container d-flex justify-content-between pr-30">
        ...
        <nav aria-label="breadcrumb">
            ...
            <ol class="breadcrumb bg-info">
              ...
              <li class="breadcrumb-item text-dark active" aria-current="page">Departement</li>
            </ol>

          </nav>
          <div class="mr-30">
            <a  href="#" data-toggle="modal" class="btn btn-primary btn-sm btn-flat btn-Add"><i class="fa fa-plus"></i> New Departement</a>
          </div>
   </div>
  ...
  <div style="margin: 50px" class="container mt-30">
    <div class="row">
        ...
        <table id="DepartementTable" class="display nowrap mt-3 w-100" style="width:100%">
            <thead>
                <tr>
                    <th>Id</th>
                    <th>Departement Stamp</th>
...
@endsection
@section('modals')
@include('admin.includes_modal.departement_modal');
@endsection
...
<script>
    ...
    var table;
    var manageTable = $('#DepartementTable');

    //  Data table population Function

    function myFunc() {
        var defaultUrl = "{{ route('admin.departements.generalList') }}";

                table = manageTable.DataTable({
                    ajax: {
                        url: defaultUrl,
                        dataSrc: 'departements'
                    },
                    searching: true,
                    // ordering: true,
                    paging: true,
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                ...
                                return "<button class='btn btn-success btn-sm btn-flat js-edit' data-id='" + data +
                                    "' data-url='/admin/departements/show/" + row.id + "'> <i class='fa fa-pen'></i>Edit</button>" +

                                    "<button class='btn btn-danger btn-sm btn-flat js-change-status ' data-id='" + data +
                                    "' data-url='/admin/departements/changeStatus/" + row.id + "'> <i class='fa fa-trash'></i>Change Status</button>";
    ...
    $('.btn-Add').on('click', function(){
        ...
        //Edit and update
                manageTable.on('click', '.js-edit', function () {
                    ...
                    $("#edit_departement_name").val(response.departement.name);
                    if(response.departement.stamp){
                    $("#stamp-placeholder").attr( 'src', '../uploads/images/stamps/' + response.departement.stamp);
                    }
                    else{
                    $("#stamp-placeholder").attr( 'src', '../admin_assets/img/ur_logo.png');
                            ...
                            // add hidden id
                            $('#id').val(response.departement.id);
                            // update - form
                ...
                manageTable.on('click', '.js-change-status', function()
                {
                    ...
                    $.ajax({
                        url: '/admin/departements/show/'+ id,
                        type: 'GET',
                        data: {id: id},
                        dataType: 'json',
                        success: function (response) {
                            ...
                            if(status == 1){
                    var html = '<form id="frmChangeStatus" action="/admin/departements/changeStatus" method="POST" >'+
                ...
                // icon: 'info',
                html:
                    '<form id="frmChangeStatus" action="admin/departements/Changestatus" method="POST" >'+
                    ...
                    $.ajax({
                        ...
                        if (data.result=="ok"){
                            ...
                            $('#add-messages').html('<div class="alert alert-success alert-message flat">' +
                                '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                                '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Depertement successfully saved. </div>');
    ...
    // Populate school depending on Colleges
                ...
                $.ajax({
                url:"{{ route('admin.departement.school_college') }}",
                type:"POST",
                data: {
                id: cat_id
                },
                success:function (data) {
                    // console.log(data);
                $('#school_id').empty();
                $.each(data.college[0].schools,function(index,school_item){

                $('#school_id').append('<option value="'+school_item.id+'">'+school_item.name+'</option>');
                })
                }
                })
                });

...
@section('destination_select','active')
@section('content')
...
<div class="content ">
    ...
    <div  class="row breadcrumb-container d-flex justify-content-between pr-30">
        ...
        <nav aria-label="breadcrumb">
            ...
            <ol class="breadcrumb bg-info ">
              ...
              <li class="breadcrumb-item text-dark active" aria-current="page">Destination</li>
            </ol>

          </nav>
          <div class="mr-30">
            <a  href="#" id="btn-Add" data-toggle="modal" class="btn btn-primary btn-sm btn-flat btn-Add"><i class="fa fa-plus"></i> New Destination</a>
          </div>
   </div>
  ...
  <div style="margin: 50px" class="container mt-30">
    <div class="row">
        ...
        <table id="DestinationTable" class="display nowrap mt-3" style="width:100%">
            <thead>
                <tr>
                    <th>Id</th>
                    {{-- <th>destination Stamp</th> --}}
                    <th> Name</th>
                    <th> email</th>
                    <th> telphone</th>
                    <th>Status</th>
...
@endsection
@section('modals')
@include('admin.includes_modal.destination_modal');
@endsection
...
<script>
var table;
var manageTable = $('#DestinationTable');

//  Data table population Function
// myFunc()

function myFunc() {
    var defaultUrl = "{{ route('admin.destinations.generalList') }}";

            table = manageTable.DataTable({
                ajax: {
                    url: defaultUrl,
                    dataSrc: 'destinations'
                },
                searching: true,
                // ordering: true,
                paging: true,
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            return "<button class='btn btn-success btn-sm btn-flat js-edit' data-id='" + data +
                                "' data-url='/admin/destinations/show/" + row.id + "'> <i class='fa fa-pen'></i>Edit</button>" +

                                "<button class='btn btn-danger btn-sm btn-flat js-change-status ' data-id='" + data +
                                "' data-url='/admin/destinations/changeStatus/" + row.id + "'> <i class='fa fa-trash'></i>Change Status</button>";
...
$('.btn-Add').on('click', function(){
    ...
    manageTable.on('click', '.js-edit', function () {
                ...
                $("#edit_name").val(response.destination.name);
                $("#edit_telphone").val(response.destination.telphone);
                $("#edit_email").val(response.destination.email);
                $("#edit_about").val(response.destination.about);

                // if(response.college.stamp){
                // $("#stamp-placeholder").attr( 'src', '../uploads/images/stamps/' + response.destination.stamp);
                // }
                // else{
                ...
                // }
                        ...
                        // add hidden id
                        $('#id').val(response.destination.id);
                        // update - form
                            ...
                            $.ajax({
                               ...
                               form[0].reset();
                                ...
                                $('#update-messages').html('<div class="alert alert-success alert-success-update">' +
                                    ...
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Destination  successfully updated. </div>');
            ...
            manageTable.on('click', '.js-change-status', function()
            {
                ...
                $.ajax({
                    url: '/admin/colleges/show/'+ id,
                    type: 'GET',
                    data: {id: id},
                    dataType: 'json',
                    success: function (response) {
                        var status = response.college.status;
                        if(status == 1){
                var html = '<form id="frmChangeStatus" action="/admin/destinations/changeStatus" method="POST" >'+
                            ...
                            $.ajax({
                               ...
                               form[0].reset();
                                ...
                                $('#changeStatus-messages').html('<div class="alert alert-success alert-success-update">' +
                                    '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Destinatio  successfully updated. </div>');
            ...
            // icon: 'info',
            html:
                '<form id="frmChangeStatus" action="admin/destination/Changestatus" method="POST" >'+
                ...
                $.ajax({
                    ...
                    if (data.result=="ok"){
                        ...
                        $('#add-messages').html('<div class="alert alert-success alert-message flat">' +
                            '<button type="button" class="close" data-dismiss="alert">&times;</button>' +
                            '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> Destination successfully saved. </div>');

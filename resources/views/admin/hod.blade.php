    ...
    <div  class="row breadcrumb-container d-flex justify-content-between pr-30">
        ...
        <nav aria-label="breadcrumb">
            ...
            <ol class="breadcrumb bg-info">
              ...
              <li class="breadcrumb-item text-dark active" aria-current="page">Head of Dept</li>
            </ol>

          </nav>
          <div class="mr-30">
            <a  href="#" data-toggle="modal" class="btn btn-primary btn-sm btn-flat btn-Add"><i class="fa fa-plus"></i>Add Head of Dpt</a>
          </div>
   </div>
...
@endsection
@section('modals')
@include('admin.includes_modal.hod_modal');
@endsection
...
<script>
    ...
    function myFunc() {
        var defaultUrl = "{{ route('admin.hod.generalList') }}";
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                ...
                                return "<button class='btn btn-success btn-sm btn-flat js-edit' data-id='" + data +
                                    "' data-url='/admin/hod/show/" + row.id + "'> <i class='fa fa-pen'></i>Edit</button>" +

                                    "<button class='btn btn-danger btn-sm btn-flat js-change-status ' data-id='" + data +
                                    "' data-url='/admin/hod/changeStatus/" + row.id + "'> <i class='fa fa-edit'></i>status</button>";
        ...
        function hideLoading(){
            ...
            // Edit and update
                manageTable.on('click', '.js-edit', function () {
                    ...
                    $("#edit_first_name").val(response.hod.first_name);
                    $("#edit_last_name").val(response.hod.last_name);
                    $("#edit_email").val(response.hod.email);
                    $("#edit_staffCode").val(response.hod.staffCode);
                ...
                manageTable.on('click', '.js-change-status', function()
                {
                    ...
                    $.ajax({
                        url: '/admin/hod/show/'+ id,
                        type: 'GET',
                        data: {id: id},
                        dataType: 'json',
                        success: function (response) {
                            var status = response.hod.status;
                            if(status == 1){
                    var html = '<form id="frmChangeStatus" action="/admin/hod/changeStatus" method="POST" >'+
                ...
                // icon: 'info',
                html:
                    '<form id="frmChangeStatus" action="admin/hod/Changestatus" method="POST" >'+

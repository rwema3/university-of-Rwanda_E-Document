    ...
    <div  class="row breadcrumb-container d-flex justify-content-between pr-30">
        ...
        <nav aria-label="breadcrumb">
            ...
            <ol class="breadcrumb bg-info">
              ...
              <li class="breadcrumb-item text-dark active" aria-current="page">Head of Finace</li>
            </ol>

          </nav>
          <div class="mr-30">
            <a  href="#" data-toggle="modal" class="btn btn-primary btn-sm btn-flat btn-Add"><i class="fa fa-plus"></i>Add Head of Finace</a>
          </div>
   </div>
...
@endsection
@section('modals')
@include('admin.includes_modal.hof_modal');
@endsection
...
<script>
    ...
    function myFunc() {
        var defaultUrl = "{{ route('admin.hof.generalList') }}";
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                ...
                                return "<button class='btn btn-success btn-sm btn-flat js-edit' data-id='" + data +
                                    "' data-url='/admin/hof/show/" + row.id + "'> <i class='fa fa-pen'></i>Edit</button>" +

                                    "<button class='btn btn-danger btn-sm btn-flat js-change-status ' data-id='" + data +
                                    "' data-url='/admin/hof/changeStatus/" + row.id + "'> <i class='fa fa-edit'></i>status</button>";
        ...
        function hideLoading(){
            ...
            // Edit and update
                manageTable.on('click', '.js-edit', function () {
                    ...
                    $("#edit_first_name").val(response.hof.first_name);
                    $("#edit_last_name").val(response.hof.last_name);
                    $("#edit_email").val(response.hof.email);
                    $("#edit_staffCode").val(response.hof.staffCode);
                                ...
                                $.ajax({
                                   ...
                                   form[0].reset();
                                    ...
                                    $('#update-messages').html('<div class="alert alert-success alert-success-update">' +
                                        ...
                                        '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong>  HOD  successfully updated. </div>');
                ...
                manageTable.on('click', '.js-change-status', function()
                {
                    ...
                    $.ajax({
                        url: '/admin/hof/show/'+ id,
                        type: 'GET',
                        data: {id: id},
                        dataType: 'json',
                        success: function (response) {
                            var status = response.hof.status;
                            if(status == 1){
                    var html = '<form id="frmChangeStatus" action="/admin/hof/changeStatus" method="POST" >'+
                ...
                // icon: 'info',
                html:
                    '<form id="frmChangeStatus" action="admin/hof/Changestatus" method="POST" >'+

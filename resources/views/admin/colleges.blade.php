...
<div class="content ">
    ...
    <div  class="row breadcrumb-container d-flex justify-content-between pr-30">
        ...
        <nav aria-label="breadcrumb">
            ...
            <ol class="breadcrumb bg-info ">
              ...
              <li class="breadcrumb-item text-dark active" aria-current="page">Colleges</li>
            </ol>

          </nav>
          <div class="mr-30">
            <a  href="#" data-toggle="modal" class="btn btn-primary btn-sm btn-flat btn-Add"><i class="fa fa-plus"></i> New College</a>
          </div>
   </div>
  ...
  <div style="margin: 50px" class="container mt-30">
    <div class="row">
        ...
        <table id="CollegeTable" class="display nowrap mt-3 " style="width:100%">
            <thead>
                <tr>
                    <th>Id</th>
                    <th>College Stamp</th>
...
@endsection
@section('modals')
@include('admin.includes_modal.college_modal');
@endsection
...
// myFunc();
function myFunc() {
    var defaultUrl = "{{ route('admin.colleges.generalList') }}";
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            return "<button class='btn btn-success btn-sm btn-flat js-edit' data-id='" + data +
                                "' data-url='/admin/colleges/show/" + row.id + "'> <i class='fa fa-pen'></i>Edit</button>" +

                                "<button class='btn btn-danger btn-sm btn-flat js-change-status ' data-id='" + data +
                                "' data-url='/admin/colleges/changeStatus/" + row.id + "'> <i class='fa fa-trash'></i>Change Status</button>";
...
$('.btn-Add').on('click', function(){
    ...
    manageTable.on('click', '.js-edit', function () {
                ...
                $("#edit_college_name").val(response.college.name);
                if(response.college.stamp){
                $("#stamp-placeholder").attr( 'src', '../uploads/images/stamps/' + response.college.stamp);
                }
                else{
                            ...
                            $.ajax({
                               ...
                               form[0].reset();
                                ...
                                $('#update-messages').html('<div class="alert alert-success alert-success-update">' +
                                    ...
                                    '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> College  successfully updated. </div>');
            ...
            manageTable.on('click', '.js-change-status', function()
            {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if(status == 1){
                var html = '<form id="frmChangeStatus" action="/admin/colleges/changeStatus" method="POST" >'+
            ...
            // icon: 'info',
            html:
                '<form id="frmChangeStatus" action="admin/college/Changestatus" method="POST" >'+
